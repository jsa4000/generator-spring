...
@WebMvcTest(<%- preffixClassName %>Controller.class)
@Import(<%- preffixClassName %>ControllerTest.DefaultConfigWithoutCsrf.class)
@ContextConfiguration(classes = {<%- preffixClassName %>ControllerTest.TestConfig.class})
@DirtiesContext(classMode = DirtiesContext.ClassMode.AFTER_EACH_TEST_METHOD)
@WithMockUser(username = "fakeuser", roles = {"FAKE"})
class <%- preffixClassName %>ControllerTest {
...
<%_ } else { _%>
        when(this.<%- preffixVariableName %>Service.find<%- preffixClassName %>Page(isNull(), isNull(), isNull(), eq(<%- preffixClassName %>Dto.<%- preffixClassName %>TypeDto.ENUM_TYPE_1), anyLong(), anyLong())).thenReturn(page<%- preffixClassName %>);
<%_ } _%>
        final var mockMvcResult = mvc.perform(get("/api/<%- projectName %>").param("type", <%- preffixClassName %>Dto.<%- preffixClassName %>TypeDto.ENUM_TYPE_1.getValue()).accept(MediaType.APPLICATION_JSON))
                .andDo(print()).andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON_VALUE)).andReturn();
        final <%- persistenceLayer ? 'PaginationResult' : 'PaginationResultDto' %><<%- preffixClassName %>Dto> apiResultList = this.objectMapper.readValue(
                mockMvcResult.getResponse().getContentAsByteArray(), new TypeReference<<%- persistenceLayer ? 'PaginationResult' : 'PaginationResultDto' %><<%- preffixClassName %>Dto>>() {
                });
<%_ if (persistenceLayer) { _%>
        verify(this.<%- preffixVariableName %>Service).findByExamplePage(any(), any(Pageable.class), eq(<%- preffixClassName %>Dto.class));
<%_ } else { _%>
        verify(this.<%- preffixVariableName %>Service).find<%- preffixClassName %>Page(isNull(), isNull(), isNull(), eq(<%- preffixClassName %>Dto.<%- preffixClassName %>TypeDto.ENUM_TYPE_1), anyLong(), anyLong());
<%_ } _%> 
        MatcherAssert.assertThat(apiResultList,
                new CustomMatcher<<%- persistenceLayer ? 'PaginationResult' : 'PaginationResultDto' %><<%- preffixClassName %>Dto>>("apiResultList") {
                    @Override
                    @SuppressWarnings("unchecked")
                    public boolean matches(Object actual) {
                        boolean matches = true;
                        final <%- persistenceLayer ? 'PaginationResult' : 'PaginationResultDto' %><<%- preffixClassName %>Dto> paginationResult = (<%- persistenceLayer ? 'PaginationResult' : 'PaginationResultDto' %><<%- preffixClassName %>Dto>) actual;
<%_ if (persistenceLayer) { _%>
                        ...
                        matches &= paginationResult.getTotalResult() == page<%- preffixClassName %>.getTotalResult();
...
<%_ } _%>
    ...
    static class TestConfig {
        ...
        @Bean
        public <%- preffixClassName %>Controller <%- preffixVariableName %>Controller(AppConfigurationProperties appConfigurationProperties,
<%_ if (persistenceLayer) { _%>
...
<%_ } _%>
            ...
            return new <%- preffixClassName %>Controller(appConfigurationProperties, <%_ if (persistenceLayer) { _%>dateToOffsetDateTimeConverter, offsetDateTimeToDateConverter, modelMapperFactory,<%_ } _%> <%- preffixVariableName %>Service);
        }
